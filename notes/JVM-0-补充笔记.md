...
<img src="JVM-0-补充笔记.assets/JVM-概述图.png" style="zoom: 80%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-JRE关系.png" style="zoom: 80%;" />
...
* Java1.8 以前的内存结构图：
  ![](JVM-0-补充笔记.assets/JVM-Java7内存结构图.png)

* Java1.8 之后的内存结果图：

  ![](JVM-0-补充笔记.assets/JVM-Java8内存结构图.png)
...
* 每个栈由多个栈帧（Frame）组成，对应着每次方法调用时所占用的内存，每个栈帧中存储着：
  ...
  * 操作数栈或表达式栈和其他一些附加信息
  ...
  <img src="JVM-0-补充笔记.assets/JVM-虚拟机栈.png" style="zoom:50%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-本地方法栈.png" style="zoom:67%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-内存图对比.png" style="zoom: 67%;" />
...
![](JVM-0-补充笔记.assets/JVM-TLAB内存分配策略.jpg)
...
![](JVM-0-补充笔记.assets/JVM-TLAB内存分配过程.jpg)
...
<img src="JVM-0-补充笔记.assets/JVM-分代收集算法.png" style="zoom: 67%;" />
...
![](JVM-0-补充笔记.assets/JVM-循环引用.png)
...
- 在可达性分析算法中，只有能够被根对象集合直接或者间接连接的对象才是存活对象
  ...
  <img src="JVM-0-补充笔记.assets/JVM-可达性分析算法.png" style="zoom: 50%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-三色标记法过程.gif" style="zoom: 67%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-三色标记法多标情况.png" style="zoom: 50%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-三色标记法漏标情况.png" style="zoom:50%;" />
...
![](JVM-0-补充笔记.assets/JVM-复制算法.png)
...
<img src="JVM-0-补充笔记.assets/JVM-标记清除算法.png" style="zoom: 67%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-标记整理算法.png" style="zoom:67%;" />
...
![](JVM-0-补充笔记.assets/JVM-垃圾回收器关系图.png)
...
![](JVM-0-补充笔记.assets/JVM-Serial收集器.png)
...
![](JVM-0-补充笔记.assets/JVM-ParNew收集器.png)
...
![](JVM-0-补充笔记.assets/JVM-ParallelScavenge收集器.png)
...
![](JVM-0-补充笔记.assets/JVM-CMS收集器.png)
...
![](JVM-0-补充笔记.assets/JVM-G1-Region区域.png)
...
<img src="JVM-0-补充笔记.assets/JUC-G1记忆集.png" style="zoom:67%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-G1回收过程.png" style="zoom: 50%;" />
...
* **Concurrent Mark **：
  ...
  * 筛选回收：并发清理阶段，首先对 CSet 中各个 Region 中的回收价值和成本进行排序，根据用户所期望的 GC 停顿时间来制定回收计划，也需要 STW
  ...
  ![](JVM-0-补充笔记.assets/JVM-G1收集器.jpg)
...
![](JVM-0-补充笔记.assets/JVM-垃圾回收器总结.png)
...
<img src="JVM-0-补充笔记.assets/JVM-对象的实际大小.png" style="zoom: 67%;" />
...
* 句柄访问：Java 堆中会划分出一块内存来作为句柄池，reference 中存储的就是对象的句柄地址，而句柄中包含了对象实例数据和类型数据各自的具体地址信息
  ...
  ![](JVM-0-补充笔记.assets/JVM-对象访问-句柄访问.png)
...
* 直接指针（HotSpot 采用）：Java 堆对象的布局必须考虑如何放置访问类型数据的相关信息，reference 中直接存储的对象地址
  ...
  ![](JVM-0-补充笔记.assets/JVM-对象访问-直接指针.png)
...
![](JVM-0-补充笔记.assets/JVM-类的生命周期.png)
...
<img src="JVM-0-补充笔记.assets/JVM-类的生命周期-加载.png" style="zoom:80%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-双亲委派模型.png" style="zoom: 50%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-热替换.png" style="zoom: 33%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-沙箱机制.png" style="zoom:67%;" />
...
![](JVM-0-补充笔记.assets/JVM-Java文件编译执行的过程.png)
...
<img src="JVM-0-补充笔记.assets/JVM-高级语言执行过程.png" style="zoom:50%;" />
...
<img src="JVM-0-补充笔记.assets/JVM-字节码指令同步控制.png" style="zoom: 33%;" />
...
* **执行引擎**开始执行字节码
  ...
  * 这里小的数字都是和字节码指令存在一起，超过 short 范围的数字存入了常量池
  ...
  ![](JVM-0-补充笔记.assets/JVM-字节码执行流程3.png)

  `istore_3`：将操作数栈顶数据弹出，存入局部变量表的 slot 3

  `getstatic #4`：获取静态字段

  ![](JVM-0-补充笔记.assets/JVM-字节码执行流程4.png)

  `iload_3`：

  ![](JVM-0-补充笔记.assets/JVM-字节码执行流程5.png)
  ...
  * 清除 main 操作数栈内容
  ...
  ![](JVM-0-补充笔记.assets/JVM-字节码执行流程6.png)
...
![](JVM-0-补充笔记.assets/JVM-执行引擎工作流程.png)
...
<img src="JVM-0-补充笔记.assets/JVM-虚方法表.png" style="zoom: 80%;" />
...
![](JVM-0-补充笔记.assets/JVM-虚方法表指向.png)
...
![](JVM-0-补充笔记.assets/JVM-jstatd图解.png)
